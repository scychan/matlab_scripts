...
%   VI111609A Vijay Iyer 11/16/09 -- Write/append to file the storedLinesPerFrame, rather than all the lines that were acquired
%
%% ***************************************************************************
...
if state.acq.saveDuringAcquisition %VI030708A
    ...
    for channelCounter = 1:state.init.maximumNumberOfInputChannels % Loop through all the channels
        if ~isempty(state.acq.acquiredData{channelCounter}(:,:,1))
            appendFrame(state.files.tifStream,state.acq.acquiredData{channelCounter}(1:state.internal.storedLinesPerFrame,:,1)); %VI111609A
        end
    end
        ...
        for channelCounter = 1:state.init.maximumNumberOfInputChannels % Loop through all the channels
            if getfield(state.acq, ['savingChannel' num2str(channelCounter)]) % If saving..
                if first % if its the first frame of first channel, then overwrite...
                    imwrite(state.acq.acquiredData{channelCounter}(1:state.internal.storedLinesPerFrame,:,frameCounter + startingFrame) ... %VI111609A
