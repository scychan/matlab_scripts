...
if state.acq.saveDuringAcquisition
    ...
    if state.acq.saveDuringAcquisition  %verifies that state hasn't changed
        ...
        if state.acq.framesPerFile %VI030609A
...
%     startChannel(state.acq.dm, state.init.eom.pockelsCellNames);
    ...
    for i = 1 : length(list)
        if state.init.eom.pockelsOn && (state.init.eom.usePowerArray || any(state.init.eom.showBoxArray) || any(state.init.eom.uncagingMapper.enabled)) %VI030708A, VI011609A
            repeatOutput=0;
        else
