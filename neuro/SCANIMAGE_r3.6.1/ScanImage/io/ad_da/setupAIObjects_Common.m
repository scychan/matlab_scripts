...
%   5/5/09 Vijay Iyer (VI050509A) - Store AI objects' uddobjects for enhancing getdata() calls
%   7/1/09 Vijay Iyer (VI070109A) - Bind a StartFcn callback to the GRAB/LOOP AI object
%   10/24/09 Vijay Iyer (VI102409A) - Bind dataMissedFcn to this event, to handle cases where AI processing fails to keep up
%
%%
...
%	set(state.init.ai, 'TriggerType', 'HwDigital');										% 6110E NI Board Set to Trigger PFI0
	...
	set(state.init.ai, 'SamplesAcquiredFcn', @makeFrameByStripes, 'TriggerFcn',@acquisitionStartedFcn, 'DataMissedFcn',@dataMissedFcn); %VI102409A %VI070109A
    if strcmpi(whichNIDriver,'DAQmx') %VI021608A
        set(state.init.ai,'HWDigitalTriggerSource',state.init.triggerInputTerminal);
    end
    state.init.aiUDD = daqgetfield(state.init.ai,'uddobject'); %VI050509A
...
%	set(state.init.aiF, 'TriggerType', 'HwDigital');										% 6110E NI Board Set to Trigger PFI0
	...
	set(state.init.aiF, 'SamplesAcquiredFcn', {'makeStripe'},'DataMissedFcn',@dataMissedFcn); %VI102409A
    state.init.aiFUDD = daqgetfield(state.init.aiF,'uddobject'); %VI050509A
