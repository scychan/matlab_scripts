...
%   1/11/09 Vijay Iyer (VI011109A) - Allow 4 input channels 
%   1/12/09 Vijay Iyer (VI011209A) - Support user-adjustable input range...
%
%% ****************************************************
global state
...
for channelCounter = 1:state.init.maximumNumberOfInputChannels
    ...
    if channelOn % if statemetnt only gets executed when there is a channel to acquire.
        ...
        offChans = [offChans channelCounter]; %VI011209A
    end
end
...
for i=1:length(AIObjects)
    ...
    if ~isempty(chans)
        ...
        for j=1:length(hwchans)
            ...
            %%%VI011209A%%%%%%%%
            voltageRange = eval(['state.acq.inputVoltageRange' num2str(hwchans{j} + 1)]);
            invert = state.acq.(['inputVoltageInvert' num2str(hwchans{j}+1)]); %VI110209A
            rangeProperties = {'InputRange' 'SensorRange'};
            for k=1:length(rangeProperties)
                set(chanobj, rangeProperties{k}, [-voltageRange voltageRange]);                
                set(chanobj, 'UnitsRange', [-1 1] * 2^(state.acq.inputBitDepth-1) * (-1)^invert); %VI110209A                
            end           
            %%%%%%%%%%%%%%%%%%%%%                
            %set(chanobj,'InputRange',[-10 10],'UnitsRange',[-10 10],'SensorRange',[-10 10]); %VI011209A
