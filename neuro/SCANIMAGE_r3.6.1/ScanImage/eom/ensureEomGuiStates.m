...
%   VI062410A Vijay Iyer 6/24/10 - BUGFIX: Use of scim_parkLaser() was causing error when 2 or more beams are used. Set voltage directly instead.
%
%% CREDITS
...
if state.init.eom.changed(beam)
    ...
    if ~state.init.eom.directMode && strcmpi(val,'FOCUS') 
        %scim_parkLaser(); %VI062410A
        setPockelsVoltage(beam, state.init.eom.lut(beam, state.init.eom.min(beam))); %VI062410A        
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%
end
