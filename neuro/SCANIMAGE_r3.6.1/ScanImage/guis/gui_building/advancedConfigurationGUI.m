...
% VI092108A Vijay Iyer 9/21/08 -- pockelsLineDelay is now a fractional parameter, not absolute time and is tied to internal line delay parameter, when computed; no remaining dependence on cusp delay, so undo VI031808A
% VI092108B Vijay Iyer 9/21/08 -- pockelsFillFraction is simply 2 extra AO samples
...
state.internal.configurationNeedsSaving=1;
genericCallback(h);
%advancedConfigurationGUI('pockelsClosedOnFlyback_Callback',gh.advancedConfigurationGUI.pockelsClosedOnFlyback); %VI031808A, VI092108A
...
if get(h, 'Value') == get(h, 'Max')
    ...
    state.acq.pockelsCellFillFraction = state.acq.fillFraction+ 2*(1/state.acq.outputRate)/state.acq.msPerLine; %VI092108B
