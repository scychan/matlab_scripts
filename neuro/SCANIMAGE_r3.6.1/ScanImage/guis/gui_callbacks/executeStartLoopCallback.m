...
%   VI050409A - Vijay Iyer 5/4/09: Check if in 'pseudofocus' mode before verifying output filename
%   VI102909A - Vijay Iyer 10/29/09: Reset repeat counter at start of LOOP operation. Also, removed reference to otherwise unused state variable.
...
if strcmp(val, 'LOOP')
	...
	if ~si_isPseudoFocus && ~savingInfoIsOK; %VI050409A
		return
	end

	% Check if file exisits
    if ~si_isPseudoFocus() %VI050409A
    ...
    state.internal.repeatsDone = 0; %VI102909A
    ...
    if state.init.eom.pockelsOn & state.init.eom.powerBoxStepper.enabled %VI011609A
