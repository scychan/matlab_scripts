...
activeZoomWarnControls = [gh.mainControls.zoomones gh.mainControls.zoomtens gh.mainControls.zoomhundreds];
...
if state.acq.zoomFactor >= state.acq.baseZoomFactor   %all's good   %VI123108A
    ...
    %%%VI012109A %%%%%%%
    idx = length(state.internal.fillFractionGUIArray) - (state.acq.baseZoomFactor - state.acq.zoomFactor);
