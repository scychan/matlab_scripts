...
if get(gh.standardModeGUI.cbSaveDuringAcq,'Value') %VI031308B
    ...
    %%%VI120108A%%%%%%%%%%%%%%%
    if state.init.eom.pockelsOn && ischar(state.init.eom.uncagingMapper.enabled) %VI011609A
        state.init.eom.uncagingMapper.enabled = ndArrayFromStr(state.init.eom.uncagingMapper.enabled);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    if state.init.eom.pockelsOn && (state.init.eom.usePowerArray || any(state.init.eom.showBoxArray) || any(state.init.eom.uncagingMapper.enabled)) %VI081308A, VI011609A
