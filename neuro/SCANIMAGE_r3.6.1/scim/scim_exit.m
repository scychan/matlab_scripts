...
for i=1:state.init.maximumNumberOfInputChannels
    if length(state.internal.GraphFigure) >= i %VI112309A
        delete(state.internal.GraphFigure(i));
    end
    
    if length(state.internal.MaxFigure) >= i %VI112309A
...
for i=1:length(daqobjs)
    if ~isempty(eval(daqobjs{i})) %VI112309A
        obj = eval(daqobjs{i});
        if isrunning(obj)
            stop(obj);
        end
